#!/bin/bash
# login-csdn.sh — 浏览器登录 CSDN 并持久化 session
#
# 使用 Playwright 的 launchPersistentContext() 保存 Chromium profile，
# 后续 auth-csdn.sh 可在 headless 模式下直接提取 cookie，无需重复登录。
#
...
log_info "打开 CSDN 登录页面。请在浏览器中登录..."

node "$SCRIPT_DIR/lib/csdn-session.cjs" login 2>&1 || {
    exit_code=$?
...
log_info "登录成功，session 已持久化到 ~/.csdn/browser-data/"
exit 0
