...
# ─── 读取内容并生成摘要 ──────────────────────────────────────
JSON_DATA=$(python3 "${SCRIPT_DIR}/lib/pipeline.py" build-juejin-draft --title "$TITLE" --content-file "$CONTENT_FILE" --category-id "$CATEGORY_ID" --tag-ids "$TAG_IDS" 2>/dev/null)

# ─── 通用请求头 ─────────────────────────────────────────────
# 掘金需要 x-csrf-token header（从 cookie 中的 passport_csrf_token 提取）
CSRF_TOKEN=$(echo "$COOKIE" | grep -o 'passport_csrf_token=[^;]*' | cut -d= -f2 || echo "")
...
PUBLISH_DATA=$(python3 "${SCRIPT_DIR}/lib/pipeline.py" build-juejin-publish --draft-id "$DRAFT_ID")
...
if data.get('err_no') == 0:
    ...
    sys.exit(1)
") || {
    ERR_MSG=$(echo "$PUBLISH_BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f\"err_no={d.get('err_no')} err_msg={d.get('err_msg')}\")" 2>/dev/null || echo "unknown")
    log_error "发布失败: $ERR_MSG"
