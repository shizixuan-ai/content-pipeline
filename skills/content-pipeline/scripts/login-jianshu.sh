#!/bin/bash
# login-jianshu.sh — 浏览器登录简书并持久化 session
#
# 使用 Playwright 的 launchPersistentContext() 保存 Chromium profile，
# 后续 auth-jianshu.sh 可在 headless 模式下直接提取 cookie，无需重复登录。
#
...
log_info "打开简书登录页面。请在浏览器中登录..."

node "$SCRIPT_DIR/lib/jianshu-session.cjs" login 2>&1 || {
    exit_code=$?
...
log_info "登录成功，session 已持久化到 ~/.jianshu/browser-data/"
exit 0
